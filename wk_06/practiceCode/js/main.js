...
const initApp = () => {
    ...
    clearItems.addEventListener("click", (event) => {
        ...
        if (list.length) {
            const confirmed = confirm("Are you sure you want to cleear the entire list?"
            );
            if(confirmed) {
                toDoList.clearList();
...
const buildListItem = (item) => {
    ...
    } else if (!item.getItemDate().includes('by')){
        label.textContent = `${item.getItem()} by ${item.getItemDate()}`;
    } else {
        label.textContent = `${item.getItem()} ${item.getItemDate()}`;
    }
...
const processSubmission = () => {
    ...
    const newEntryTextDate = getNewEntryDate();
    if (!newEntryText.length) return;
    const nextItemId = calcNextItemId();
    const toDoItem = createNewItem(nextItemId, newEntryText, newEntryTextDate);
...
const getNewEntryDate = () => {
    return document.getElementById("newItemDate").value.trim();
};
